...
@section('title', 'Current-Stock-Position-Report-Blank')
...
<div class="col-sm-12">
    ...
    @endphp
        @foreach ($table->groupBy(['GodownName']) as $godownName => $godownItems)
            ...
            @endphp
            @foreach ($godownItems->groupBy(['DepartmentName']) as $DepartmentName => $items)
                <tr>
               ...
               @php
                   ...
                   $amount = ($item->Amount == 0 || $item->Amount == '') ? round($item->OurPrice * $item->StockQty, 2) : round($item->Amount, 2);
                   $departmentSubtotal += $amount; // Increment the subtotal variable for the category group
               ...
               <tr>
                   ...
                   <td class="text-right">{{ round($item->BasePrice, 2) }}</td>
                   <td class="text-right">{{ round($item->FixedPrice, 2) }}</td>
                   <td class="text-right"><b>{{ round($item->StockQty, 2) }}</b></td>
           ...
           @endforeach
            ...
            @endforeach
            <tr>
                <th class="text-right text-blue" colspan="10">SubTotal....  {{ $DepartmentName }} :</th>
