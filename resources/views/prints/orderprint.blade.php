...
@section('title', 'OrderEntry-Print')
...
<div class=""> <div class="row col-sm-12 ml-1">
    <div class="col-sm-5">
         <img class="img-fluid "  src="<?php echo url('images/Branches/'.config('app.BranchPicture')); ?>" style="max-width:100px"> <div class="row pb-2"><div class="col-sm-12"> <h2><strong> {{$company->CompanyName}} </strong></h2> </div>
</div> <div class="row">
     <div class="col-sm-12">
    ...
    <h6>WebSeite : <a href="//{{$company->WebsiteAddress}}" target="_blank">{{$company->WebsiteAddress}}</a></h6>
</div> </div> </div>
<div class="col-sm-1">
    ...
    <div class="col-sm-6">
        ...
        <div class="clearfix"></div>
         ...
         <div class="card shadow border border-dark line col-sm-5">
            <p class="mt-2">{{$OrderMasterData->EventNo}}</p>
        </div>
    </div>
        <div class="row custom-hight">
            <div class="card shadow border border-dark line mr-1 col-sm-5">
                <p class="mt-2">Cust. Ref# :</p>
            </div><div class="card shadow border border-dark line mr-1 col-sm-5">
                <p class="mt-2">{{$OrderMasterData->CustomerRefNo}}</p>
            </div>
            </div>
                <div class="row custom-hight">
            <div class="card shadow border border-dark line mr-1 col-sm-5">
            ...
            <div class="card shadow border border-dark line col-sm-5">
                <p class="mt-2">{{date('d-M-Y', strtotime($OrderMasterData->OrderDate))}}</p>
            </div>
         </div>
         <div class="row custom-hight">
            <div class="card shadow border border-dark line mr-1 col-sm-5">
                <p class="mt-2">Department.</p>
            </div>
            <div class="card shadow border border-dark line  col-sm-5">
                <p class="mt-2">{{$OrderMasterData->DepartmentName}}</p>
            </div>
        </div>
        <div class="row custom-hight">
            <div class="card shadow border border-dark line mr-1 col-sm-5">
            ...
            <div class="card shadow border border-dark  line col-sm-5">
                <p class="mt-2">{{$OrderMasterData->PortName}}</p>
            </div>
        </div>
    </div>
</div>
         ...
         <div class="row py-2  ml-1">
            ...
            <div class="row mx-1 ">{{$OrderMasterData->CustomerName}}</div>
            <div class="row mx-1 ">
                 <p>{{$Customercodedata->Address}}</p>
            </div>
        </div><div class="custom-col-5  border border-dark rounded mr-auto ">
            <div class="row mx-1 ">
            <b>Vessel :</b>
        </div>
            <div class="row mx-1 ">{{$OrderMasterData->VesselName}}</div>
        </div>
    </div>
     ...
     <div class="rounded shadow mx-auto">
        ...
        @foreach ($OrderDetails as $item)
            ...
            <tr>
                    ...
                    <td class="sellprice text-right" id="sellprice<?php echo $counter;?>" value="{{round($item->Price,2)}}">{{round($item->Price,2)}}</td>
