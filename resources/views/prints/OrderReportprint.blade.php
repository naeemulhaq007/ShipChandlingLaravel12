    ...
    <tbody id="OrderReporttablebody">
                    ...
                    @foreach ($orders as $item)
                        <tr>
                            ...
                            <td style="width: 80px">{{ $item->PONo ? $item->PONo : '' }}</td>
                            <td style="width: 250px">{{ $item->OrderDate ? date('d-M-Y',strtotime($item->OrderDate)) : '' }}</td>
                            <td style="width: 100px">{{ $item->VSNNo ? $item->VSNNo : '' }}</td>
                            <td style="width: 100px">{{ $item->EventNo ? $item->EventNo : '' }}</td>
                            <td style="width: 600px">{{ $item->CustomerName ? $item->CustomerName : '' }}</td>
                            <td style="width: 300px">{{ $item->VesselName ? $item->VesselName : '' }}</td>
                            <td >{{ $item->Department ? $item->Department : '' }}</td>
                            <td >{{ $item->Terms ? $item->Terms : '' }}</td>
                            <td >{{ $item->OrderQty ? round($item->OrderQty,2) : 0 }}</td>
                            <td >{{ $item->RecQty ? round($item->RecQty,2) : 0 }}</td>
                            <td >{{ $item->DeliveredQty ? round($item->DeliveredQty,2) : 0 }}</td>
                            <td >{{ $item->RtnQty ? round($item->RtnQty,2) : 0 }}</td>
                            <td >{{ $item->MissingQty ? round($item->MissingQty,2) : 0 }}</td>
                            <td >{{ $item->SoldQty ? round($item->SoldQty,2) : 0 }}</td>
                            <td >{{ $item->SaleReturnQty ? round($item->SaleReturnQty,2) : 0 }}</td>
                            <td >{{ $item->ExtAmount ? round($item->ExtAmount,2) : 0 }}</td>
                            <td >{{ $item->ChkSendEmail ? 'Yes' : 'No' }}</td>
                        </tr>
                    @endforeach
